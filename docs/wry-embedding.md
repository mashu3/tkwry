...
|----|--------------|------------------|---------------------|
| **macOS** | `WKWebView` as `NSView` subview | Added under the given `NSView` | Toplevel content `NSView` via `TkMacOSXGetRootControl` (`tkwry/_parent.py`) — child `Frame`s usually share one view; each WebView is wrapped in a clip container (`NSView` with `wantsLayer` / `masksToBounds` / `clipsToBounds`) |
