"""Unit coverage for create-time clipboard= opt-in."""
...
def test_clipboard_default_false(tk_root) -> None:
    frame = tk.Frame(tk_root)
    web = WebView(frame, html="<p>c</p>")
    assert web.clipboard is False
    web.destroy()
    assert web.clipboard is False
    frame.destroy()


def test_clipboard_true_readable_after_destroy(tk_root) -> None:
    frame = tk.Frame(tk_root)
    web = WebView(frame, html="<p>c</p>", clipboard=True)
...
def test_try_create_passes_clipboard(tk_root, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    web = WebView(frame, html="<p>c</p>", width=200, height=150, clipboard=True)
    ...
    web._try_create()
    assert captured.get("clipboard") is True
    web.destroy()
    frame.destroy()
